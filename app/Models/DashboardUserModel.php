...
class DashboardUserModel extends Model
{
    ...
    private function getStats(int $userId, float $imc): array
    {
        ...
        $activitesSemaine = 0;
        if ($this->safeTableExists('suggestions_activites') && $userId > 0) {
            $row = $this->db->table('suggestions_activites')
    ...
    private function getRegimesProgress(int $userId): array
    {
        ...
        $rows = $this->db->table('abonnements_regimes ar')
            ->select('r.nom, r.variation_poids, ar.date_debut, ar.date_fin, ar.statut')
        ...
        foreach ($rows as $row) {
            ...
            $variation = isset($row['variation_poids']) ? (float) $row['variation_poids'] : 0.0;
            $result[] = [
                'nom' => $row['nom'] ?? 'Regime',
    ...
    {
        if (!$this->safeTableExists('historique_transactions') || $userId <= 0) {
            return [];
        }
        ...
        foreach ($rows as $row) {
            ...
            $result[] = [
                ...
                'detail' => number_format((float) ($row['montant'] ?? 0), 2) . ' EUR',
            ];
        }
    ...
    {
        $sources = [
            ['table' => 'user_weight_tracking', 'date' => 'date', 'weight' => 'poids', 'user' => 'utilisateur_id'],
            ['table' => 'suivi_poids', 'date' => 'date_suivi', 'weight' => 'poids', 'user' => 'utilisateur_id'],
            ['table' => 'historique_poids', 'date' => 'date_suivi', 'weight' => 'poids', 'user' => 'utilisateur_id'],
            ['table' => 'poids_journalier', 'date' => 'date_jour', 'weight' => 'poids', 'user' => 'utilisateur_id'],
        ];

        foreach ($sources as $source) {
            if (!$this->safeTableExists($source['table']) || $userId <= 0) {
                continue;
            }

            $fields = $this->db->getFieldNames($source['table']);
            if (
                !in_array($source['date'], $fields, true) ||
                !in_array($source['weight'], $fields, true) ||
                !in_array($source['user'], $fields, true)
            ) {
                continue;
            }

            $rows = $this->db->table($source['table'])
                ->select($source['date'] . ' AS day, ' . $source['weight'] . ' AS weight')
                ->where($source['user'], $userId)
                ->orderBy($source['date'], 'ASC')
                ->get(14)
                ->getResultArray();
            ...
            if (!empty($rows)) {
                return array_map(static function ($row): array {
                    return [
                        'day' => date('d/m', strtotime((string) ($row['day'] ?? 'now'))),
    ...
    {
        if (
            !$this->safeTableExists('abonnements_regimes') ||
            !$this->safeTableExists('regimes') ||
            $userId <= 0
        ) {
            return [
                'nom' => 'Regime actuel',
    ...
    private function buildCaloriesSeries(): array
    {
        return [
            ...
            ['month' => 'Dec', 'value' => 2050],
            ['month' => 'Jan', 'value' => 1820],
            ['month' => 'Fev', 'value' => 1760],
    ...
    private function formatPeriod(string $start, string $end): string
    {
        ...
        return $mois[(int) date('n', $s) - 1] . ' - ' . $mois[(int) date('n', $e) - 1] . ' ' . date('Y', $e);
    }
}
